...
import { LayoutDashboard, CheckSquare, BarChart2, Calendar, Settings, Command, Wallet, Shield, Users, LogOut } from 'lucide-react';
...
const Sidebar = () => {
  ...
  if (user?.role === 'admin') {
    menuItems = [
      { icon: LayoutDashboard, label: 'System Overview', path: '/admin/dashboard' },
      { icon: Settings, label: 'Admin Settings', path: '/admin/settings' },
    ];
  } else if (user?.role === 'manager') {
    menuItems = [
      { icon: LayoutDashboard, label: 'Team Dashboard', path: '/manager/dashboard' },
